...
import { type SignInState, signInAction } from "../actions/sign-in.action";
import { type SignInInput, signInDefaultValues, signInSchema } from "../schemas/sign-in.schema";
...
export function LoginForm({ redirectTo }: LoginFormProps) {
  ...
  const form = useForm<SignInInput>({
    ...
    defaultValues: { ...signInDefaultValues, redirect: redirectTo },
  });
          ...
          <form action={formAction} className="space-y-4">
            <FieldGroup>
              <div className="flex flex-col items-center gap-1 text-center">
                ...
                <p className="text-sm text-balance text-muted-foreground">{t("signInSubtitle")}</p>
              </div>
              ...
              {state?.error && (
                <p className="text-sm text-destructive">{tErrors(state.error as never)}</p>
              )}

              <Button type="submit" disabled={pending} className="w-full">
                {pending ? <Loader2Icon className="animate-spin" /> : t("submitSignIn")}
              </Button>
